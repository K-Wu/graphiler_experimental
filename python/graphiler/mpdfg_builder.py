...
def mpdfg_builder(msg_func, reduce_func, update_func=None, opt_level=2):
    ...
    # extra_params_str_list = [str(param) for param in extra_params if str(param) not in extra_params_int_type else str(param)+":int"]
    # mpdfg_func = FuncTemplate.replace(
    #    "__extra__", ",".join(extra_params_str_list))
    mpdfg_func = FuncTemplate.replace(
    ...
    update_func = torch.jit.script(update_func).inlined_graph if update_func else None
